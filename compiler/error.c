...
extern char *yytext;
extern ErrorDefinition zec_error_message_format[];
               ...
               va_list ap)
{
    ...
    wc_format = zec_mbstowcs_alloc(line_number, format->format);
    DBG_assert(wc_format != NULL, ("wc_format is null.\n"));
    
    for (i = 0; wc_format[i] != L'\0'; i++) {
        if (wc_format[i] != L'$') {
            zec_vwstr_append_character(v, wc_format[i]);
            continue;
        }
        ...
        case INT_MESSAGE_ARGUMENT:
            ...
            zec_vwstr_append_string(v, wc_buf);
            break;
        case DOUBLE_MESSAGE_ARGUMENT:
...
self_check()
{
    if (strcmp(zec_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("compile error message format error.\n"));
    }
    if (strcmp(zec_error_message_format
...
void
zec_compile_error(int line_number, CompileError id, ...)
{
    va_list     ap;
    VWString    message;
    ...
    zec_vwstr_clear(&message);
    format_message(line_number,
                   &zec_error_message_format[id],
                   &message, ap);
    fprintf(stderr, "%s:%3d:", zec_get_current_compiler()->path,
            line_number);
