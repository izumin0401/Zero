...
ErrorDefinition zec_error_message_format[] = {
    {"dummy"},
    {"文法エラー($(token)付近)"},
    {"不正な文字($(bad_char))"},
    {"関数名が重複しています($(name))"},
    ...
    {"abstractなメソ\ッドは実装を持てません。"},
    {"abstractでないメソ\ッドには実装がなければなりません。"},
    ...
    {"コンストラクタとして指定されているメンバ$(member_name)は"
     "メソ\ッドではありません。"},
    {"newで指定されているメソ\ッド$(member_name)がコンストラクタでは"
     "ありません。"},
    {"現状でフィールド($(member_name))は呼び出せません。"},
    {"メソ\ッド$(member_name)に代入しようとしています。"},
    {"virtualでないメソ\ッド($(name))はオーバーライドできません。"},
    {"メソ\ッドをオーバーライドする際はoverrideの指定が必要です($(name))。"},
    {"abstractでないクラスに、abstractメソ\ッド$(method_name)が存在します。"},
    {"スーパークラスがいないクラスでsuperを使っています。"},
    {"superはメソ\ッドを呼ぶ以外使用できません。"},
    {"superのフィールドは参照できません。"},
    {"$(name)はフィールドです。オーバーライドできません。"},
    {"フィールド名$(name)が重複しています。"},
    {"配列に$(name)というメソ\ッドはありません。"},
    {"文字列に$(name)というメソ\ッドはありません。"},
    {"instanceofのオペランドは参照型でなければなりません。"},
    {"instanceofの右辺の型は参照型でなければなりません。"},
    {"instanceofの対象はクラスでなければなりません。"},
    {"このinstanceofは常に真です。"},
    {"このinstanceofは常に偽です。"},
    {"Zeroのインターフェイスには親子関係はないので、"
     "このinstanceofは常に偽です。"},
    ...
    {"オーバーライドしているメソ\ッド$(name)のアクセス修飾子が、"
     "スーパークラスに比べ狭くなっています。"},
    {"メソ\ッドまたは関数$(name)の引数の数が間違っています。"},
    {"メソ\ッドまたは関数$(func_name)の$(index)番目の引数"
     "$(param_name)の型が間違っています。"},
    {"メソ\ッドまたは関数$(name)の戻り値の型が不正です。"},
    {"メソ\ッドまたは関数$(func_name)が、適合しない例外"
     "$(exception_name)をthrowsに記述しています。"},
    {"コンストラクタを直接呼び出すことはできません。"},
    {"型名$(name)が見付かりません。"},
    {"列挙型$(type_name)に$(enumerator_name)はありません。"},
    {"Zeroではインターフェイスは継承できません(今のところ)。"},
