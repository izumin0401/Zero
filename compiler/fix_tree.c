...
static int
reserve_function_index(ZEC_Compiler *compiler, FunctionDefinition *src)
{
    int i;
    ZVM_Function *dest;
    ...
    for (i = 0; i < compiler->zvm_function_count; i++) {
        if (zvm_compare_package_name(src_package_name,
            ...
            && !strcmp(src->name,
                       compiler->zvm_function[i].name)) {
            MEM_free(src_package_name);
            return i;
        }
    }
    compiler->zvm_function
        = MEM_realloc(compiler->zvm_function,
                      sizeof(ZVM_Function) * (compiler->zvm_function_count+1));
    dest = &compiler->zvm_function[compiler->zvm_function_count];
    ...
    return compiler->zvm_function_count - 1;
}
...
add_class(ClassDefinition *src)
{
    ...
    for (i = 0; i < compiler->zvm_class_count; i++) {
        if (zvm_compare_package_name(src_package_name,
            ...
            && !strcmp(src->name,
                       compiler->zvm_class[i].name)) {
            MEM_free(src_package_name);
            return i;
        }
    }
    compiler->zvm_class
        = MEM_realloc(compiler->zvm_class,
                      sizeof(ZVM_Class) * (compiler->zvm_class_count+1));
    dest = &compiler->zvm_class[compiler->zvm_class_count];
    compiler->zvm_class_count++;

    dest->package_name = src_package_name;
    dest->name = MEM_strdup(src->name);
    dest->is_implemented = ZVM_FALSE;

    return compiler->zvm_class_count - 1;
}
...
is_exception_class(ClassDefinition *cd)
{
    ...
    exception_class = zec_search_class(EXCEPTION_CLASS_NAME);
...
fix_throws(ExceptionList *throws)
{
    ...
    for (pos = throws; pos; pos = pos->next) {
        pos->ref->class_definition = zec_search_class(pos->ref->identifier);
        if (pos->ref->class_definition == NULL) {
            zec_compile_error(pos->ref->line_number,
...
fix_type_specifier(TypeSpecifier *type)
{
    ...
    if (type->basic_type == ZVM_UNSPECIFIED_IDENTIFIER_TYPE) {
        cd = zec_search_class(type->identifier);
        if (cd) {
            if (!zec_compare_package_name(cd->package_name,
                                          compiler->package_name)
                && cd->access_modifier != ZVM_PUBLIC_ACCESS) {
                zec_compile_error(type->line_number,
...
create_function_derive_type(FunctionDefinition *fd)
{
    ...
    ret = zec_alloc_type_specifier(fd->type->basic_type);
    *ret = *fd->type;
    ret->derive = zec_alloc_type_derive(FUNCTION_DERIVE);
...
fix_identifier_expression(Block *current_block, Expression *expr)
{
    ...
    decl = zec_search_declaration(expr->u.identifier.name, current_block);
    if (decl) {
        expr->type = decl->type;
        expr->u.identifier.is_function = ZVM_FALSE;
        expr->u.identifier.u.declaration = decl;
        return expr;
    }
    fd = zec_search_function(expr->u.identifier.name);
    if (fd == NULL) {
        zec_compile_error(expr->line_number,
    ...
    expr->u.identifier.is_function = ZVM_TRUE;
...
cast_mismatch_error(int line_number, TypeSpecifier *src, TypeSpecifier *dest)
{
    ...
    tmp = zec_get_type_name(src);
    src_name = zec_strdup(tmp);
    MEM_free(tmp);

    tmp = zec_get_type_name(dest);
    dest_name = zec_strdup(tmp);
    MEM_free(tmp);
...
{
    Expression *cast_expr = zec_alloc_expression(CAST_EXPRESSION);
    ...
    if (cast_type == INT_TO_DOUBLE_CAST) {
        cast_expr->type = zec_alloc_type_specifier(ZVM_DOUBLE_TYPE);
    } else if (cast_type == DOUBLE_TO_INT_CAST) {
        cast_expr->type = zec_alloc_type_specifier(ZVM_INT_TYPE);
        ...
        cast_expr->type = zec_alloc_type_specifier(ZVM_STRING_TYPE);
    } else {
               ...
               ClassDefinition *dest_interface, int interface_index)
{
    ...
    type = zec_alloc_type_specifier(ZVM_CLASS_TYPE);
    ...
    cast_expr = zec_alloc_expression(UP_CAST_EXPRESSION);
    cast_expr->type = type;
...
check_type_compatibility(TypeSpecifier *super_t, TypeSpecifier *sub_t)
{
    ...
    if (!zec_is_class_object(super_t)) {
        return zec_compare_type(super_t, sub_t);
    }
    if (!zec_is_class_object(sub_t)) {
        return ZVM_FALSE;
    }
             ...
             ExceptionList **error_exception)
{
    ...
    ZVM_Boolean is_interface_dummy;
    int interface_index_dummy;
    ZVM_Boolean is_thrown = ZVM_TRUE;
...
create_assign_cast(Expression *src, TypeSpecifier *dest)
{
    ...
    if (zec_compare_type(src->type, dest)) {
        return src;
    }

    if (zec_is_object(dest)
        && src->type->basic_type == ZVM_NULL_TYPE) {
        DBG_assert(src->type->derive == NULL, ("derive != NULL"));
        return src;
    }

    if (zec_is_class_object(src->type) && zec_is_class_object(dest)) {
    ...
    if (zec_is_function(src->type) && zec_is_delegate(dest)) {
        TypeSpecifier *dele_type;
        DelegateDefinition *dd;

        dele_type = zec_alloc_type_specifier(src->type->basic_type);
        *dele_type = *(src->type);
    ...
    if (zec_is_int(src->type) && zec_is_double(dest)) {
        cast_expr = alloc_cast_expression(INT_TO_DOUBLE_CAST, src);
        return cast_expr;
    } else if (zec_is_double(src->type) && zec_is_int(dest)) {
        cast_expr = alloc_cast_expression(DOUBLE_TO_INT_CAST, src);
        return cast_expr;
    } else if (zec_is_string(dest)) {
                      ...
                      ExceptionList **el_p)
{
    ...
    if (left->kind == IDENTIFIER_EXPRESSION
        ...
        zec_compile_error(left->line_number,
...
eval_math_expression_int(Expression *expr, int left, int right)
{
    ...
    expr->type = zec_alloc_type_specifier(ZVM_INT_TYPE);
...
eval_math_expression_double(Expression *expr, double left, double right)
{
    ...
    expr->type = zec_alloc_type_specifier(ZVM_DOUBLE_TYPE);
...
{
    ZVM_Char *left_str = expr->u.binary_expression.left->u.string_value;
    ZVM_Char *right_str;
    int         len;
    ZVM_Char    *new_str;

    right_str = zec_expression_to_string(expr->u.binary_expression.right);
    if (!right_str) {
        return expr;
    }
    
    len = zvm_wcslen(left_str) + zvm_wcslen(right_str);
    new_str = MEM_malloc(sizeof(ZVM_Char) * (len + 1));
    zvm_wcscpy(new_str, left_str);
    zvm_wcscat(new_str, right_str);
    MEM_free(left_str);
    MEM_free(right_str);
    expr->kind = STRING_EXPRESSION;
    expr->type = zec_alloc_type_specifier(ZVM_STRING_TYPE);
...
cast_binary_expression(Expression *expr)
{
    ...
    if (zec_is_int(expr->u.binary_expression.left->type)
        && zec_is_double(expr->u.binary_expression.right->type)) {
        cast_expression
            = alloc_cast_expression(INT_TO_DOUBLE_CAST, 
                                    expr->u.binary_expression.left);
        expr->u.binary_expression.left = cast_expression;

    } else if (zec_is_double(expr->u.binary_expression.left->type)
               && zec_is_int(expr->u.binary_expression.right->type)) {
        cast_expression
            = alloc_cast_expression(INT_TO_DOUBLE_CAST, 
                                    expr->u.binary_expression.right);
    ...
    if (zec_is_string(expr->u.binary_expression.left->type)
        && expr->kind == ADD_EXPRESSION) {
                           ...
                           ExceptionList **el_p)
{
    ...
    if (zec_is_int(expr->u.binary_expression.left->type)
        ...
        expr->type = zec_alloc_type_specifier(ZVM_DOUBLE_TYPE);
               ...
               && ((zec_is_string(expr->u.binary_expression.left->type)
                    && zec_is_string(expr->u.binary_expression.right->type))
                   || (zec_is_string(expr->u.binary_expression.left->type)
...
eval_compare_expression_boolean(Expression *expr,
                                ZVM_Boolean left, ZVM_Boolean right)
{
    ...
    expr->type = zec_alloc_type_specifier(ZVM_BOOLEAN_TYPE);
...
eval_compare_expression_string(Expression *expr,
                               ZVM_Char *left, ZVM_Char *right)
{
    int cmp;

    cmp = zvm_wcscmp(left, right);
                       ...
                       ExceptionList **el_p)
{
    ...
    if (!(zec_compare_type(expr->u.binary_expression.left->type,
                           expr->u.binary_expression.right->type)
          || (zec_is_object(expr->u.binary_expression.left->type)
              && expr->u.binary_expression.right->kind == NULL_EXPRESSION)
          || (zec_is_object(expr->u.binary_expression.right->type)
                              ...
                              ExceptionList **el_p)
{
    ...
    if (zec_is_boolean(expr->u.binary_expression.left->type)
        && zec_is_boolean(expr->u.binary_expression.right->type)) {
        expr->type = zec_alloc_type_specifier(ZVM_BOOLEAN_TYPE);
    } else {
        zec_compile_error(expr->line_number,
                     ...
                     ExceptionList **el_p)
{
    ...
    if (!zec_is_int(expr->u.minus_expression->type)
        && !zec_is_double(expr->u.minus_expression->type)) {
        zec_compile_error(expr->line_number,
                           ...
                           ExceptionList **el_p)
{
    ...
    if (!zec_is_boolean(expr->u.logical_not->type)) {
        zec_compile_error(expr->line_number,
               ...
               ExceptionList **el_p, TypeSpecifier *array_base)
{
    ...
    for (param = param_list;
        ...
        if (param->type->basic_type == ZVM_BASE_TYPE) {
...
add_exception(ExceptionList **el_p, ExceptionList *throws)
{
    ...
    for (prev = NULL, pos = throws; pos; prev = pos, pos = pos->next) {
        new_item = zec_malloc(sizeof(ExceptionList));
                             ...
                             ExceptionList **el_p)
{
    ...
    if (func_expr->kind == IDENTIFIER_EXPRESSION) {
        fd = zec_search_function(func_expr->u.identifier.name);

    } else if (func_expr->kind == MEMBER_EXPRESSION) {
        if (zec_is_array(func_expr->u.member_expression.expression->type)) {
        ...
        } else if (zec_is_string(func_expr->u.member_expression.expression
                                 ->type)) {
            ...
            if (func_expr->u.member_expression.declaration->kind
                == FIELD_MEMBER) {
                zec_compile_error(expr->line_number, FIELD_CAN_NOT_CALL_ERR,
    ...
    if (zec_is_delegate(func_expr->type)) {
    ...
    check_argument(current_block, expr->line_number,
                   ...
                   array_base_p);
    expr->type = zec_alloc_type_specifier(func_type->basic_type);
    *expr->type = *func_type;
    expr->type->derive = func_type->derive;
    if (func_type->basic_type == ZVM_CLASS_TYPE) {
                            ...
                            Expression *obj, char *member_name)
{
    ...
    member = zec_search_member(obj->type->u.class_ref.class_definition,
                               member_name);
    if (member == NULL) {
        zec_compile_error(expr->line_number,
    ...
    if (compiler->current_class_definition == NULL
        ...
        if (member->access_modifier == ZVM_PRIVATE_ACCESS) {
            zec_compile_error(obj->line_number,
    ...
    if (!zec_compare_package_name(compiler->package_name,
                                  obj->type->u.class_ref.class_definition
                                  ->package_name)
        && member->access_modifier != ZVM_PUBLIC_ACCESS)
    {
        zec_compile_error(obj->line_number,
...
create_first_enumerator(EnumDefinition *ed)
{
    ...
    expr = zec_alloc_expression(ENUMERATOR_EXPRESSION);
    expr->type = zec_alloc_type_specifier(ZVM_ENUM_TYPE);
                      ...
                      ExceptionList **el_p)
{
    ...
    if (expr->u.member_expression.expression->kind == IDENTIFIER_EXPRESSION
        && (ed = zec_search_enum(expr->u.member_expression
...
fix_this_expression(Expression *expr)
{
    ...
    cd = zec_get_current_compiler()->current_class_definition;
    if (cd == NULL) {
        zec_compile_error(expr->line_number,
...
fix_super_expression(Expression *expr, Expression *parent)
{
    ...
    if (parent == NULL || parent->kind != MEMBER_EXPRESSION) {
        zec_compile_error(parent->line_number,
                             ...
                             ExceptionList **el_p)
{
    ...
    expr->type = zec_alloc_type_specifier(elem_type->basic_type);
    *expr->type = *elem_type;
    expr->type->derive = zec_alloc_type_derive(ARRAY_DERIVE);
                     ...
                     ExceptionList **el_p)
{
    ...
    if (!zec_is_int(ie->index->type)) {
        zec_compile_error(expr->line_number,
                          INDEX_NOT_INT_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    expr->type = zec_alloc_type_specifier2(ie->array->type);
                       ...
                       ExceptionList **el_p)
{
    ...
    if (!zec_is_int(expr->u.inc_dec.operand->type)) {
        zec_compile_error(expr->line_number,
                          ...
                          ExceptionList **el_p)
{
    ...
    if (!zec_is_object(operand->type)) {
        zec_compile_error(expr->line_number,
                          INSTANCEOF_OPERAND_NOT_REFERENCE_ERR,
                          MESSAGE_ARGUMENT_END);
    }

    if (!zec_is_object(target)) {
        zec_compile_error(expr->line_number,
                          INSTANCEOF_TYPE_NOT_REFERENCE_ERR,
                          MESSAGE_ARGUMENT_END);
    }

    if (!zec_is_class_object(operand->type) || !zec_is_class_object(target)) {
        zec_compile_error(expr->line_number,
                          INSTANCEOF_FOR_NOT_CLASS_ERR,
                          MESSAGE_ARGUMENT_END);
    }

    if (zec_compare_type(operand->type, target)) {
        zec_compile_error(expr->line_number,
                         ...
                         ExceptionList **el_p)
{
    ...
    if (!zec_is_class_object(org_type)) {
        zec_compile_error(expr->line_number,
                          DOWN_CAST_OPERAND_IS_NOT_CLASS_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    if (!zec_is_class_object(target_type)) {
        zec_compile_error(expr->line_number,
                          DOWN_CAST_TARGET_IS_NOT_CLASS_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    if (zec_compare_type(org_type, target_type)) {
        zec_compile_error(expr->line_number,
                   ...
                   ExceptionList **el_p)
{
    ...
    member = zec_search_member(expr->u.new_e.class_definition,
        ...
        if (dim_pos->expression) {
            dim_pos->expression
                = fix_expression(current_block, dim_pos->expression, expr,
                                 el_p);
            if (!zec_is_int(dim_pos->expression->type)) {
                zec_compile_error(expr->line_number,
                                  ARRAY_SIZE_NOT_INT_ERR,
                                  MESSAGE_ARGUMENT_END);
            }
        }
        tmp_derive = zec_alloc_type_derive(ARRAY_DERIVE);
        tmp_derive->next = derive;
        derive = tmp_derive;
    }

    expr->type = zec_alloc_type_specifier2(expr->u.array_creation.type);
               ...
               ExceptionList **el_p)
{
    ...
    case NULL_EXPRESSION:
        expr->type = zec_alloc_type_specifier(ZVM_NULL_TYPE);
        break;
                 ...
                 FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    if (!zec_is_boolean(if_s->condition->type)) {
        zec_compile_error(if_s->condition->line_number,
        ...
        for (expr_pos = case_pos->expression_list; expr_pos;
            ...
            expr_pos->expression
                = fix_expression(current_block, expr_pos->expression,
                                 NULL, el_p);
            if (!(zec_compare_type(switch_s->expression->type,
                                   expr_pos->expression->type)
                  || (zec_is_object(switch_s->expression->type)
                      && expr_pos->expression->kind == NULL_EXPRESSION)
                  || (zec_is_object(expr_pos->expression->type)
                      && switch_s->expression->kind == NULL_EXPRESSION))) {
                zec_compile_error(expr_pos->expression->line_number,
                    ...
                    FunctionDefinition *fd, ExceptionList **el_p)
{
    while_s->condition
        = fix_expression(current_block, while_s->condition, NULL,
                         el_p);
    if (!zec_is_boolean(while_s->condition->type)) {
        zec_compile_error(while_s->condition->line_number,
                  ...
                  FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    if (for_s->condition != NULL
        && !zec_is_boolean(for_s->condition->type)) {
        zec_compile_error(for_s->condition->line_number,
                       ...
                       FunctionDefinition *fd, ExceptionList **el_p)
{
    do_while_s->condition
        = fix_expression(current_block, do_while_s->condition, NULL,
                         el_p);
    if (!zec_is_boolean(do_while_s->condition->type)) {
        zec_compile_error(do_while_s->condition->line_number,
                     ...
                     FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    if (fd->type->derive == NULL && fd->type->basic_type == ZVM_VOID_TYPE
        && return_value != NULL) {
    ...
    if (return_value == NULL) {
        if (fd->type->derive) {
            if (fd->type->derive->tag == ARRAY_DERIVE) {
                return_value = zec_alloc_expression(NULL_EXPRESSION);
            } else {
            ...
            case ZVM_VOID_TYPE:
                return_value = zec_alloc_expression(INT_EXPRESSION);
                return_value->u.int_value = 0;
                break;
            case ZVM_BOOLEAN_TYPE:
                return_value = zec_alloc_expression(BOOLEAN_EXPRESSION);
                return_value->u.boolean_value = ZVM_FALSE;
                break;
            case ZVM_INT_TYPE:
                return_value = zec_alloc_expression(INT_EXPRESSION);
                return_value->u.int_value = 0;
            ...
            case ZVM_DOUBLE_TYPE:
                return_value = zec_alloc_expression(DOUBLE_EXPRESSION);
...
add_declaration(Block *current_block, Declaration *decl,
                ...
                ZVM_Boolean is_parameter)
{
    if (zec_search_declaration(decl->name, current_block)) {
        zec_compile_error(line_number,
    ...
    if (current_block) {
        current_block->declaration_list
            = zec_chain_declaration(current_block->declaration_list, decl);
    }
    if (fd) {
        decl->is_local = ZVM_TRUE;
        add_local_variable(fd, decl, is_parameter);
    } else {
        ZEC_Compiler *compiler = zec_get_current_compiler();
        decl->is_local = ZVM_FALSE;
        compiler->declaration_list
            = zec_chain_declaration(compiler->declaration_list, decl);
    }
}
                  ...
                  FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    for (catch_pos = try_s->catch_clause; catch_pos;
        ...
        if (!zec_is_class_object(catch_pos->type)) {
            zec_compile_error(catch_pos->line_number,
        ...
        decl = zec_alloc_declaration(ZVM_TRUE,
                                     catch_pos->type,
...
type_to_exception_list(TypeSpecifier *type, int line_number)
{
    ...
    el = zec_malloc(sizeof(ExceptionList));
    el->ref = zec_malloc(sizeof(ExceptionRef));
    el->next = NULL;
                    ...
                    FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    if (statement->u.throw_s.exception) {
        ...
        if (!zec_is_class_object(statement->u.throw_s.exception->type)) {
            zec_compile_error(statement->line_number,
        ...
        current_catch = zec_get_current_compiler()->current_catch_clause;
    ...
    for (block_p = block; block_p; block_p = block_p->outer_block) {
        if (block_p->type == FINALLY_CLAUSE_BLOCK) {
            ...
            if (block_p->type == WHILE_STATEMENT_BLOCK) {
                if (zvm_compare_string(block_p->parent.statement.statement
              ...
              FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    case DECLARATION_STATEMENT:
        add_declaration(current_block, statement->u.declaration_s, fd,
                        statement->line_number, ZVM_FALSE);
...
add_parameter_as_declaration(FunctionDefinition *fd)
{
    ...
    for (param = fd->parameter; param; param = param->next) {
        if (zec_search_declaration(param->name, fd->block)) {
            zec_compile_error(param->line_number,
                              PARAMETER_MULTIPLE_DEFINE_ERR,
                              STRING_MESSAGE_ARGUMENT, "name", param->name,
                              MESSAGE_ARGUMENT_END);
        }
        fix_type_specifier(param->type);
        decl = zec_alloc_declaration(ZVM_TRUE, param->type, param->name);
        if (fd == NULL || fd->block) {
...
add_return_function(FunctionDefinition *fd, ExceptionList **el_p)
{
    ...
    ret = zec_create_return_statement(NULL);
    ...
    *last_p = zec_create_statement_list(ret);
}
...
fix_function(FunctionDefinition *fd)
{
    ...
    if (!check_throws(fd->throws, el, &error_exception)) {
        zec_compile_error(fd->end_line_number,
...
add_super_interfaces(ClassDefinition *cd)
{
    ...
    for (super_pos = cd->super_class; super_pos;
        ...
        for (if_pos = super_pos->interface_list; if_pos;
             if_pos = if_pos->next) {
            ExtendsList *new_extends = zec_malloc(sizeof(ExtendsList));
            *new_extends  = *if_pos;
...
{
    ExtendsList *e_pos;
    ExtendsList *e_pos_prev = NULL;
    ClassDefinition *super;
    int dummy_class_index;

    if (cd->class_or_interface == ZVM_INTERFACE_DEFINITION
        && cd->extends != NULL) {
        zec_compile_error(cd->line_number,
    ...
    for (e_pos = cd->extends; e_pos; e_pos = e_pos->next) {
        ...
        if (super->class_or_interface == ZVM_CLASS_DEFINITION) {
            if (cd->super_class) {
                zec_compile_error(cd->line_number,
            ...
            if (e_pos_prev) {
                e_pos_prev->next = e_pos->next;
            } else {
                cd->extends = cd->extends->next;
            }
        }
        e_pos_prev = e_pos;
    }
    cd->interface_list = cd->extends;
...
add_default_constructor(ClassDefinition *cd)
{
    ...
    type = zec_alloc_type_specifier(ZVM_VOID_TYPE);
    block = zec_alloc_block();
    modifier = zec_create_class_or_member_modifier(VIRTUAL_MODIFIER);
    if (cd->super_class) {
        statement = zec_alloc_statement(EXPRESSION_STATEMENT);
        super_e = zec_create_super_expression();
        member_e = zec_create_member_expression(super_e,
                                                DEFAULT_CONSTRUCTOR_NAME);
        func_call_e = zec_create_function_call_expression(member_e, NULL);
        statement->u.expression_s = func_call_e;
        block->statement_list = zec_create_statement_list(statement);
        modifier = zec_create_class_or_member_modifier(VIRTUAL_MODIFIER);
        modifier2 = zec_create_class_or_member_modifier(OVERRIDE_MODIFIER);
        modifier = zec_chain_class_or_member_modifier(modifier, modifier2);
        modifier3 = zec_create_class_or_member_modifier(PUBLIC_MODIFIER);
        modifier = zec_chain_class_or_member_modifier(modifier, modifier3);
    } else {
        block->statement_list = NULL;
    }
    fd = zec_create_function_definition(type, DEFAULT_CONSTRUCTOR_NAME,
                                        NULL, NULL, block);
    if (tail) {
        tail->next = zec_create_method_member(&modifier, fd, ZVM_TRUE);
    } else {
        cd->member = zec_create_method_member(&modifier, fd, ZVM_TRUE);
    }
}
...
{
    if ((super_method->access_modifier == ZVM_PUBLIC_ACCESS
         && sub_method->access_modifier != ZVM_PUBLIC_ACCESS)
        || (super_method->access_modifier == ZVM_FILE_ACCESS
            && sub_method->access_modifier == ZVM_PRIVATE_ACCESS)) {
        zec_compile_error(sub_method->line_number,
...
static void
fix_class_list(ZEC_Compiler *compiler)
{
    ...
    for (class_pos = compiler->class_definition_list;
         class_pos; class_pos = class_pos->next) {
        if (class_pos->class_or_interface != ZVM_CLASS_DEFINITION)
            continue;
    ...
    for (class_pos = compiler->class_definition_list;
            ...
            if (member_pos->kind == METHOD_MEMBER) {
                ...
                if (super_member) {
                    if (super_member->kind != METHOD_MEMBER) {
                        zec_compile_error(member_pos->line_number,
                                          FIELD_OVERRIDED_ERR,
                                          STRING_MESSAGE_ARGUMENT, "name",
        ...
        if (abstract_method_name && !class_pos->is_abstract) {
            zec_compile_error(class_pos->line_number,
...
void
fix_enum_list(ZEC_Compiler *compiler)
{
    ...
    for (ed_pos = compiler->enum_definition_list; ed_pos;
         ed_pos = ed_pos->next) {
        ed_pos->index = enum_count;
        enum_count++;
    }
}

void
fix_delegate_list(ZEC_Compiler *compiler)
{
...
void
zec_fix_tree(ZEC_Compiler *compiler)
{
